...
def preprocess_function(example):
    disability_label = example['disability_sig']
    labels = [0. for i in range(len(unique_labels))]
    if disability_label == 1:
        label = "Disability - significant objective"
        labels[label2id[label]] = 1.
    elif disability_label >= 2:
        label = "Disability - principal objective"
        labels[label2id[label]] = 1.
...
dataset = dataset.map(preprocess_function, remove_columns=['disability_sig'])
