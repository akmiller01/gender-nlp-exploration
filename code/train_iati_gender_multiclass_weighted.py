...
count = Counter()
count.update(dataset['gender_equality_sig'])
print(count)

# Remove some of the full negative examples
negative = dataset.filter(lambda example: example['gender_equality_sig'] == 0)
positive = dataset.filter(lambda example: example['gender_equality_sig'] != 0)
negative = negative.shuffle(seed=42).select(range(math.floor(positive.num_rows)))
...
# Stratify and split
dataset = dataset.add_column('class_labels', dataset['gender_equality_sig'])
...
def preprocess_function(example):
    gender_label = example['gender_equality_sig']
    labels = [0. for i in range(len(unique_labels))]
    if gender_label == 1:
        label = "Gender equality - significant objective"
        labels[label2id[label]] = 1.
    elif gender_label >= 2:
        label = "Gender equality - principal objective"
        labels[label2id[label]] = 1.
...
dataset = dataset.map(preprocess_function, remove_columns=['gender_equality_sig'])
...
training_args = TrainingArguments(
    'iati-gender-multi-classifier-weighted',
