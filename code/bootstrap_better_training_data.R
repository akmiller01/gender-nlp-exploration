...
dat = read.csv(
  "large_data/iati_predictions.csv",
  check.names=F,
  colClasses = list(
    character=c(
      "text",
...
training_data$label = "No gender equality objective"
training_data$label[which(training_data$sig == 1)] = "Significant gender equality objective"
training_data$label[which(training_data$sig == 2)] = "Principal gender equality objective"

keep = c("text", "label")
training_data = training_data[,keep]
fwrite(training_data, "large_data/training_data.csv")
