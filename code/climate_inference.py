...
def main():
    ...
    dataset_screened = dataset[
        (dataset['climate_adaptation'].isin([2]) | dataset['climate_mitigation'].isin([2]))
    ]
    dataset_text = dataset_screened[text_cols]
    # De-duplicate
    ...
    dataset_screened = pd.merge(dataset_screened, dataset_text, on=text_cols, how="left")
    dataset_screened.to_csv('large_data/crs_screened_climate_predictions.csv', index=False)
