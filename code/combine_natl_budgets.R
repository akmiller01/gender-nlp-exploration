...
uga$value = rowSums(uga[,c("value.a", "value.b")])
uga[,c("value.a", "value.b")] = NULL
...
# CC
ggplot(subset(dat, label=="Climate change"), aes(x=label, y=percentage, group=country, fill=country)) + 
  geom_bar(stat="identity", position="dodge") +
  scale_fill_manual(values=reds) +
  scale_y_continuous(expand = c(0, 0), labels=percent) +
  di_style +
  labs(
    y="Percent of total national budget\n(various years)",
    x="",
    fill=""
  )
# G
ggplot(subset(dat, label=="Gender"), aes(x=label, y=percentage, group=country, fill=country)) + 
  geom_bar(stat="identity", position="dodge") +
  scale_fill_manual(values=reds) +
  scale_y_continuous(expand = c(0, 0), labels=percent) +
  di_style +
...
# D
ggplot(subset(dat, label=="Disability"), aes(x=label, y=percentage, group=country, fill=country)) + 
  geom_bar(stat="identity", position="dodge") +
  scale_fill_manual(values=reds) +
  scale_y_continuous(expand = c(0, 0), labels=percent) +
  di_style +
